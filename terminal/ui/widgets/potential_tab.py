...
class PotentialTab(QWidget):
    ...
    def _on_double_clicked(self, index) -> None:
        ...
        row = index.row()
        if not hasattr(self, "_rows") or row >= len(self._rows):
            return
        r = self._rows[row]
        sym, ivl = r[1], r[2]
        try:
            ...
            from PySide6.QtWidgets import QDialog, QLabel as QLab, QVBoxLayout, QScrollArea
            from PySide6.QtGui import QPixmap

            client = MexcClient()
            klines = client.klines(sym, ivl, 500)
            png = render_potential_chart(sym, ivl, klines)
            if png is None:
                return
            dlg = QDialog(self)
            dlg.setWindowTitle(f"{sym} {ivl} — Potansiyel {r[3]}")
