"""OKX demo dashboard — okx_trades tablosunu gösteren mobil web (salt-okunur).

Paper web'inin (terminal.web.server) OKX karşılığı. okx_trades + canlı OKX
bakiyesini okur. Harmonik performansı + açık/kapalı işlemler + sıralanabilir
tablolar. Ayrı port (varsayılan 8090).

Kullanım:
    OKX_API_KEY=.. OKX_SECRET=.. OKX_PASSPHRASE=.. WEB_PASS=gizli \\
    python -m terminal.web.okx_server --port 8090
"""
...
h1{font-size:18px;color:#26c6da;margin:4px 0 12px}
...
.sec{color:#26c6da;font-weight:700;margin:14px 0 6px}
.sec.dim{color:#888}
...
table.sortable th.asc::after{content:" \\2191";color:#26c6da;font-size:10px}
table.sortable th.desc::after{content:" \\2193";color:#26c6da;font-size:10px}
"""
...
def render(db_path, refresh: int) -> str:
    ...
    try:
        ...
        try:
            rows_open = conn.execute(
                ...
                "FROM okx_trades o LEFT JOIN setups s ON s.id=o.setup_id "
            ...
            pat_rows = conn.execute(
                ...
                "FROM okx_trades WHERE closed_at IS NOT NULL "
            ...
            acc = conn.execute(
                ...
                "SUM(COALESCE(pnl_usd,0)) FROM okx_trades WHERE closed_at IS NOT NULL"
            ).fetchone()
            if acc:
    ...
    cards = f"""<div class="cards">
      ...
      <div class="card"><div class="lbl">Açık</div><div class="val" style="color:#26c6da">{len(rows_open)}</div></div>
    </div>"""

    return f"""<!doctype html><html lang="tr"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<meta http-equiv="refresh" content="{refresh}">
<title>OKX Demo · Harmonik</title><style>{STYLE}</style></head>
<body><div class="wrap">
<h1>🟦 OKX Demo — Harmonik (gerçek demo borsa)</h1>
{cards}
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            if not self._auth():
                ...
                self.send_header("WWW-Authenticate", 'Basic realm="okx"')
                self.end_headers()
                return
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(prog="terminal-okx-web",
                                 description="OKX demo dashboard (salt-okunur)")
    ap.add_argument("--host", default=os.environ.get("WEB_HOST", "0.0.0.0"))
    ap.add_argument("--port", type=int, default=int(os.environ.get("OKX_WEB_PORT", "8090")))
    ...
    log.info("OKX web dashboard: http://%s:%d (db: %s)", args.host, args.port, args.db)
    try:
        httpd.serve_forever()
